 ...
 */
public class Common1_2BannerAdapter extends BannerAdapter<ProductListBean.Detail, Common1_2BannerAdapter.ItemViewHolder> {
    Context context;
    String type;

    public Common1_2BannerAdapter(Context context, List<ProductListBean.Detail> detail, String type) {
        super(detail);
    ...
    public ItemViewHolder onCreateHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_banner1_2, parent, false);
    ...
    public void onBindView(ItemViewHolder holder, ProductListBean.Detail data, int position, int size) {
        if (data != null) {
            ...
            holder.tv1.setText(data.getProductName());
